...
class FirewallRule(forms.Form):
    ...
    src_port = forms.IntegerField(min_value=0, max_value=65535, label='Source Port', widget=wgt_port, required=False)
    dst_ip = forms.IPAddressField(label='Destination IP', required=False)
    dst_port = forms.IntegerField(min_value=0, max_value=65535, label='Destination Post', widget=wgt_port, required=False)
